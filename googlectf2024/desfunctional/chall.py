...
class Desfunctional:
    def __init__(self):
        self.key = os.urandom(24)  # Always remain the same
    ...
    def corruption(self):
        ...
        self.flipped_bits = self.flipped_bits.union(
            random.choices(remaining, k=num_flips))  # Each time choose some new bits to add to the set of flip bits  
        mask = int.to_bytes(sum(2**i for i in self.flipped_bits), 24)
        return bytes(i ^ j for i, j in zip(self.key, mask))   # The new key is the constant key flipping some bits
